      ...
      <Card className="w-full max-w-sm border border-slate-200 shadow-sm">
        <Card.Content className="p-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              {touched.email && !emailValid && email.length > 0 && !loading ? (
                ...
                <p className="text-xs text-red-500">
                  Already registered.{' '}
                  <HeroLink href="/sign-in" className="underline">
                    Sign in instead?
                  </HeroLink>
                </p>
              ) : (
            ...
            <Button type="submit" isDisabled={loading || usernameAvailable === false || emailTaken} variant="primary">
              {loading ? 'Creating account…' : 'Create account'}
            </Button>
          </form>
          <p className="mt-4 text-center text-sm text-slate-500">
            Already have an account?{' '}
            <HeroLink href="/sign-in" className="text-blue-600">
              Sign in
            </HeroLink>
          </p>
        </Card.Content>
